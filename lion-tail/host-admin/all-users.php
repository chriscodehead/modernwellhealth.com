...
if (isset($_GET['unblock']) && !empty($_GET['unblock'])) {
  ...
  $email = $cal->selectFrmDB($user_tb, 'email', 'id', $_GET['unblock']);
  $amount = $cal->selectFrmDB($user_tb, 'amount', 'id', $_GET['unblock']);
  $date = $bassic->getDate();
  $fullname = $cal->selectFrmDB($user_tb, 'first_name', 'id', $_GET['unblock']) . ' ' . $cal->selectFrmDB($user_tb, 'last_name', 'id', $_GET['unblock']);
  $email_call->loanApproval($email, $amount, $fullname, $date);
  if (!empty($blockuser)) {
    $msg = $blockuser;
  }
